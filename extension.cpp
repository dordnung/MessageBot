...
void WatchThread::RunThread(IThreadHandle *pHandle) {
    ...
    while (messageBot.extensionLoaded) {
        ...
        if (messageBot.extensionLoaded && !messageBot.messageQueue.empty()) {
            ...
            CallBackResult result = messageBot.webClass->SendMessageWebAPI(message.username, message.password, message.message, message.showLogin, messageBot.recipients, messageBot.debugEnabled);
